...
> **Warning: gviz silently falls back to the first tab.** If `--tab` names a tab
> that does not exist, Google returns the *first* sheet instead of an error, and
> the import will happily read the wrong data. Check the reported player count
> and the "first 15" line against what you expect.
...
```
[11:52:03] HEADS UP — pick 20 is 3 away.
           Likely: WR A.J. Brown (64% survives)
[11:52:31] ON THE CLOCK — pick 20
           TAKE: WR A.J. Brown (64% survives)
[11:53:43] STILL OPEN after 72s — pick 20
           TAKE: WR A.J. Brown (64% survives)
[11:53:58] CONFIRMED pick 20: WR A.J. Brown. Nice one.
```

**Every message names at most one player.** No shortlist, no alternatives, no
reasoning. These messages get shared in a public channel, and a shortlist or a
line like "2 left in RB T2; 0% survives to pick 20" tells the other eleven
managers exactly what you are thinking.
...
**That also means `HEADS UP` barely fires.** With instant bots, every intervening
pick lands inside one poll cycle, so the bot goes straight from "you just
picked" to "you are on the clock" and never observes the one-to-three-away
window. It fired once in fourteen picks. Not a defect — but a bot-filled mock
cannot exercise that path, and testing it needs a human-paced room.
...
WhatsApp numbers and a Discord channel, so nobody needs a terminal open. The
copy is identical to the console's; the only difference is who may see what
(see *Audiences* below).
...
| `TWILIO_ACCOUNT_SID` / `TWILIO_AUTH_TOKEN` / `TWILIO_WHATSAPP_FROM` / `TWILIO_WHATSAPP_TO_PRIVATE` / `TWILIO_WHATSAPP_TO_PUBLIC` | Twilio equivalent (needs a paid account) |
| `DISCORD_WEBHOOK_URL` | Channel → Settings → Integrations → Webhooks |

**Audiences.** The private rendering carries the survival forecast — the one
number a rival most wants — and goes only to `TO_PRIVATE`. The relay's DM and
the Discord channel get the public rendering: one name, nothing about the
board. On the runner the console log is also world-readable (public repo), so
the workflow passes `--console-audience public`.
...
| `npm run board:import -- --sheet <id> --tab "<tab>"` | Sheet → `config/board.json` |
