...
| Cron guard is a date check inside the job, not just the cron expression | GitHub cron has no year field; `40 18 5 9 *` would refire 2027-09-05. A step exits the job unless `date -u +%F` equals `2026-09-05` (dispatch runs are never guarded) | 2026-08-30 |
